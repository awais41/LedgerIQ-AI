...
if page == "Dashboard":
    ...
    cols = st.columns(5)
    with cols[0]: metric("Revenue", money(summary["revenue"]), "Total recorded income")
    with cols[1]: metric("Expenses", money(summary["expenses"]), "Total recorded spending")
    with cols[2]: metric("Net Cashflow", money(summary["net_cashflow"]), "Revenue minus expenses")
    with cols[3]: metric("Expense Ratio", f"{expense_ratio:.0f}%", "Expenses as % of revenue")
    with cols[4]: metric("Anomalies", f"{len(anomalies):,}", "Transactions requiring review")
